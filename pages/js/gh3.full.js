...
(function () {
	...
	Gh3.access_token = ''; // added by wayne chan, 2015/08/05
	...
	Gh3.Helper = Kind.extend({
		...
		callHttpApi : function (apiParams) {
		  var sUrl = Gh3.Helper.protocol + "://" + Gh3.Helper.domain + "/" + apiParams.service;
		  if (Gh3.access_token) // added by wanye at 2015/08/05
		    sUrl += (sUrl.indexOf('?') > 0?'&':'?') + 'access_token=' + Gh3.access_token;
			apiParams.url = sUrl;
		...
		fetchContent : function (callback) {
			...
			Gh3.Helper.callHttpApi({
				...
				success : function(res) {
					...
					that.sha = res.data.sha;   // added by wayne chan, 2015/07/24
					that.size = res.data.size; // added by wayne chan, 2015/07/24
